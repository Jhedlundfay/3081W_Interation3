 ...
 ******************************************************************************/
/**
 * @brief The parent class from which all Arena entities inherit.
 *
 ...
 * will be reset if the robot loss condition is met. There is no way to win.
 *
 ...
 */
  ...
  /**
   * @brief the setter for the entities' intensities. The values used by the
   * robots' sensors to determine how the robot should be moving.
   * By making this property universal to all entities, the sensors are
   * open to extension.
   */
  void set_intensity(double intensity) {
    intensity_ = intensity;
    set_color({static_cast<int>(255*(intensity/1200.0)),
    static_cast<int>(255*(intensity/1200.0)),
    static_cast<int>(255*(intensity/1200.0))});
  }

  /**
   * @brief the getter for the entities' intensities. Used by the
   * sensors to calculate the impulse based on distance from the entity.
   */
  double get_intensity() { return intensity_; }

 private:
  double intensity_{1200.0};
